...
if [[ $answer = y ]] ; then
	...
	echo
	seq_rarefact=$(grep "Min" results/library_summary_cc.txt | cut -d":" -f2)
	echo "Computing alpha and beta diversity. Rarefaction is set to ${seq_rarefact} sequences based on the Min number of sequences in your library_summary_cc.txt file."
